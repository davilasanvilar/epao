...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const isVideoStr = formData.get('isVideo') as string;
    const isVideo = isVideoStr === 'true' ? 1 : 0;
    const videoUrl = isVideo ? (formData.get('url') as string) : null;
    ...
    await turso.execute({
      sql: "INSERT INTO resources (name, isVideo, url, description, img) VALUES (?, ?, ?, ?, ?)",
      args: [name, isVideo, videoUrl, description, img]
    });
...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const sqlCount = `SELECT COUNT(*) as count FROM resources`;
    const countResult = await turso.execute(sqlCount);
    const totalCount = countResult.rows[0].count as number;

    const sqlData = `SELECT * FROM resources ORDER BY created_at DESC LIMIT ? OFFSET ?`;
    ...
    const resources = await Promise.all(dataResult.rows.map(async row => ({
      id: row.id,
      name: row.name,
...
export const PUT: APIRoute = async ({ request }) => {
  try {
    ...
    const currentResource = await turso.execute({ sql: "SELECT img FROM resources WHERE id = ?", args: [id] });
    let currentImg = currentResource.rows[0]?.img as string;
    ...
    await turso.execute({
      sql: "UPDATE resources SET name = ?, isVideo = ?, url = ?, description = ?, img = ? WHERE id = ?",
      args: [name, isVideo, videoUrl, description, currentImg, id]
    });
...
export const DELETE: APIRoute = async ({ request }) => {
  try {
    ...
    const currentResource = await turso.execute({ sql: "SELECT img FROM resources WHERE id = ?", args: [id] });
    const currentImg = currentResource.rows[0]?.img as string;
    ...
    if (currentImg) {
      ...
      try {
        ...
        console.error("Failed to delete image from B2 on resource delete", e);
      }
    }
